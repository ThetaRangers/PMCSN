...
double getService(enum node_type type, int id, int shengen)
{
	...
	case SECURITY:
		//Truncated Normal
		if(shengen) {
			return TruncatedNormal(SECURITY_MEAN / 2.0, 2, 1.5, 5);
		} else {
			return TruncatedNormal(SECURITY_MEAN * 2.0, 2, 1.5, 5);
		}
...
double finite_horizon(int mode, int n0[3], int n1[3], int n2[3], int n3[3])
{
	...
	for(int j = 0; j < 4; j++) {
		...
		if (t.current == t.arrival) {
			...
			case TEMP:
				dequeue(&servers[type][id].head, &servers[type][id].tail,
					&pass_type, &arrival, &shengen);

				if (servers[type][id].number > 0)
					servers[type][id].completion =
						t.current +
						getService(type, id, shengen);
				else
					servers[type][id].completion = INFINITY;
			...
			case SECURITY:
				...
				if(servers[type][id].head == NULL) {
					shengen = servers[type][id].head->shengen;
				} else {
					shengen = servers[type][id].head_second->shengen;
				}*/
